...
func init() {
	// SSS mainnet Address encoding magics
	...
	// SSS testnet Address encoding magics
...
// SSSParser handle
type SSSParser struct {
	*btc.BitcoinParser
	baseparser                         *bchain.BaseParser
	BitcoinOutputScriptToAddressesFunc btc.OutputScriptToAddressesFunc
}

// NewSSSParser returns new SSSParser instance
func NewSSSParser(params *chaincfg.Params, c *btc.Configuration) *SSSParser {
	p := &SSSParser{
...
// GetChainParams contains network parameters for the main SSS network
...
func (p *SSSParser) ParseBlock(b []byte) (*bchain.Block, error) {
	r := bytes.NewReader(b)
	w := wire.MsgBlock{}
	h := wire.BlockHeader{}
	err := h.Deserialize(r)
	...
	if h.Version > 3 {
		// Skip past AccumulatorCheckpoint which was added in sss block version 4
...
func (p *SSSParser) PackTx(tx *bchain.Tx, height uint32, blockTime int64) ([]byte, error) {
	return p.baseparser.PackTx(tx, height, blockTime)
}

// UnpackTx unpacks transaction from protobuf byte array
func (p *SSSParser) UnpackTx(buf []byte) (*bchain.Tx, uint32, error) {
	return p.baseparser.UnpackTx(buf)
}

// ParseTx parses byte array containing transaction and returns Tx struct
func (p *SSSParser) ParseTx(b []byte) (*bchain.Tx, error) {
	t := wire.MsgTx{}
	r := bytes.NewReader(b)
...
func (p *SSSParser) TxFromMsgTx(t *wire.MsgTx, parseAddresses bool) bchain.Tx {
...
func (p *SSSParser) ParseTxFromJson(msg json.RawMessage) (*bchain.Tx, error) {
	var tx bchain.Tx
...
func (p *SSSParser) outputScriptToAddresses(script []byte) ([]string, bool, error) {
...
func (p *SSSParser) GetAddrDescForUnknownInput(tx *bchain.Tx, input int) bchain.AddressDescriptor {
	if len(tx.Vin) > input {
...
func (p *SSSParser) GetValueSatForUnknownInput(tx *bchain.Tx, input int) *big.Int {
	if len(tx.Vin) > input {
		scriptHex := tx.Vin[input].ScriptSig.Hex
		if scriptHex != "" {
			script, _ := hex.DecodeString(scriptHex)
...
func (p *SSSParser) GetValueSatFromZerocoinSpend(signatureScript []byte) (*big.Int, error) {
